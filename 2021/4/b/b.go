...
func main() {
	...
	addGrid(gridNumber, currentGrid)
	completedGridNumber, calledNumber := findLastCompletedGrid(nbrs)
	sum := 0
	...
	log.Print("Grid ", completedGridNumber, " with ", sum, "=>", calledNumber)
...
func findLastCompletedGrid(numbers []string) (lastGrid int, lastNumber string) {
	var tmpRow = make([][5]int, len(grids))
	var tmpCol = make([][5]int, len(grids))
	var wonGrids []int
	for _, number := range numbers {
		for gridIndex, grid := range grids {
			if len(wonGrids) == len(grids) {
				return
			}
			if funk.ContainsInt(wonGrids, gridIndex) {
				continue
			}
		breakLoop:
			for colIndex := 0; colIndex < 5; colIndex++ {
				for rowIndex := 0; rowIndex < 5; rowIndex++ {
					...
					if tmpCol[gridIndex][rowIndex] == 5 || tmpRow[gridIndex][rowIndex] == 5 {
						lastGrid = gridIndex
						lastNumber = number
						wonGrids = append(wonGrids, gridIndex)
						break breakLoop
					}
				}
